...
const NotificationList = () => {
  const { notifications, set: setNotificationsStore } = useNotificationStore(
    (s) => s
  );
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-20 flex items-end px-4 py-6 sm:p-6">
      <div className="flex w-full flex-col">
        {reversedNotifications.map((notification, idx) => (
          <Notification
            key={`${notification.message}${idx}`}
             ...
             onHide={() => {setNotificationsStore((state: any) => {
              const reversedIndex = reversedNotifications.length-1-idx;
              state.notifications = [...notifications.slice(0 , reversedIndex),
        ...
        <div className="p-4">
          <div className="flex items-center">
            <div className="flex-shrink-0">
              ...
              {type === "error" ? (<XCircleIcon className="text-error mr-1 h-8 w-8"/>) :"text-error mr-1 h-8 w-8"}
                <div className="ml-2 w-0 flex-1">
                  <div className="text-fgd-1 font-bold">
                      {message}
                  </div>
                    {description ? ( <p className="text-fgd-2 mt-0.5 text-sm">
                      {description}
                    </p>) : null}
                  {
                    txid ? (
                    <div className="flex flex-row">
                      <a href={`https://explorer.solana.com/tx/` + txid + `?cluster=${networkConfiguration}`} 
                          target="_blank" rel="noreferrer" className="link-accent link flex flex-grow">
                            <NotificationSVG/>
